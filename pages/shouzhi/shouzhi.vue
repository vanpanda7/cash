	...
	export default {
		...
		methods: {
			...
			fetchAllData() {
				...
				Promise.all(dayReqsOut).then(data => { this.weekOutData = data; this.$nextTick(() => this.drawChart('canvasOut', data, '#FD416E')) })
				Promise.all(dayReqsIn).then(data => { this.weekInData = data; this.$nextTick(() => this.drawChart('canvasIn', data, '#4D7BF3')) })
			},
